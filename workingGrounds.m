...
mdotsteps=(m_dot_range(1,2)-m_dot_range(1,1))/m_dot_range(1,3)+1;

total_loops=p_steps*t_steps*eqv_steps*mdotsteps;
...
Output_dataNames=["I/P Pressure [Pa]","I/P Temperature [K]","Eqv Ratio","I/P Density [kg/m^3]","Speed of Sound (in that gas mix) [m/s]",...
                    ...
                    "ISP_e [s]", "ISP_alt_mdot [s]","Mass Flow Rate [kg/s]"];
sz=0;%initializes the size fcn, so it doesnt error out

%% Data Aq. Loop

n=0;
for m=m_dot_range(1,1):m_dot_range(1,3):m_dot_range(1,2)
    for p=Pressure_range(1,1):Pressure_range(1,3):Pressure_range(1,2)
        for t=Temp_range(1,1):Temp_range(1,3):Temp_range(1,2)
            for e=eqv_ratio_range(1,1):eqv_ratio_range(1,3):eqv_ratio_range(1,2)
                ...
                T_e = m*(sqrt(2*cp1*T1))*(sqrt(1 + Term_1b - Term_2b * Term_3b * Term_4b));
                Tsp_e = T_e/m;
                ...
                Output(sz(1,1)+1,:)=[p,t,e,density(gas_i),spd_sound_gas,CJ_spd,VN_pressure,...
                                    ...
                                    T_e,Tsp_e,Thrust_altmdot,Tspe_altmdot,Isp_e,ISP_altmdot,m];
    
                sz=size(Output);
                fprintf('\n \nlooptime: %d, loop number: %d of %d',toc,n,total_loops)
                save('Output_data_dec17_vm_dot','Output',"Output_dataNames") %% saves the file as .mat file for later accessing saving here incase it errors out, overwrite each loop
            end 
        end
    end
end