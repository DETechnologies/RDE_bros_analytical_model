...
eqv_ratio_range=[1,1,0.005];  % low,high,step size %no units
m_dotRange=[0.1,2,0.01];% low,high,step size %kg/s
...
Output_dataNames=["I/P Pressure [Pa]","I/P Temperature [K]","Eqv Ratio","I/P Density [kg/m^3]","Speed of Sound (in that gas mix) [m/s]",...
                    ...
                    "ISP_e [s]", "ISP_alt_mdot [s]","m_dot"];
sz=0;%initializes the size fcn, so it doesnt error out

%% Data Aq. Loop

n=0;
for mdot=m_dotRange(1,1):m_dotRange(1,3):m_dotRange(1,2)
        ...
        for t=Temp_range(1,1):Temp_range(1,3):Temp_range(1,2)
            for e=eqv_ratio_range(1,1):eqv_ratio_range(1,3):eqv_ratio_range(1,2)
                ...
                Output(sz(1,1)+1,:)=[p,t,e,density(gas_i),spd_sound_gas,CJ_spd,VN_pressure,...
                                    ...
                                    T_e,Tsp_e,Thrust_altmdot,Tspe_altmdot,Isp_e,ISP_altmdot,mdot];
    
                sz=size(Output);
                fprintf('\n \nlooptime: %d, loop number: %d of %d',toc,n,total_loops)
                save('Output_data_jan04_vMdot_LP','Output',"Output_dataNames") %% saves the file as .mat file for later accessing saving here incase it errors out, overwrite each loop
            end 
        end
    end
end 
