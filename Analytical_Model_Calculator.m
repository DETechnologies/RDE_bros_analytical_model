...
Pressure_range=[1114575,6.08e+6,101.325e+3]; % low,high,step size % in pascals (range 5 atm to 25 atm (500psi))
Temp_range=[300,320,1]; % low,high,step size % in Kelvin
eqv_ratio_range=[0.9,1.2,0.1];  % low,high,step size %no units
...
for p=Pressure_range(1,1):Pressure_range(1,3):Pressure_range(1,2)
    for t=Temp_range(1,1):Temp_range(1,3):Temp_range(1,2)
        for e=eqv_ratio_range(1,1):eqv_ratio_range(1,3):eqv_ratio_range(1,2)
            ...
            save('Output_data_dec10_new_pt2','Output',"Output_dataNames") %% saves the file as .mat file for later accessing saving here incase it errors out, overwrite each loop
        end 
    end
end
